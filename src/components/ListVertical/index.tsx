...
import { MessageOutlined, LikeOutlined, BorderBottomOutlined, LikeFilled } from '@ant-design/icons';
    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(result)) {
          ...
          message.error('未能成功获取帖子列表');
        }
      } catch (error) {
        console.error('获取帖子列表时出现错误:', error);
        message.error('获取帖子列表时出现错误');
      }
    };
          ...
          renderItem={(item) => (
            ...
            >
              <List.Item.Meta
                ...
                description={
                  <span>
                    {item.user && <Tag color="geekblue">{item.user.gender}</Tag>}
