...
export async function deleteUser(params: API.UserDeleteRequest) {
  return request<API.BaseResponse<boolean>>(`/api/user/delete/` + params.id, {
    method: 'DELETE',
    // params: { ...params },
    headers: {
      'Content-Type': 'application/json',
 ...
 * @param params
 */
export async function updateLoginUser(params: API.CurrentUser, options?: { [key: string]: any }) {
  return request<API.BaseResponse<number>>(`/api/user/updateCurrent`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
